...
def handler(event, context):
    ...
    try:
        ...
        if typ == "JPEG" or "TIFF" or "PNG" or "BMP":
            ori = Image.open(fin).convert("RGBA")
            new = Image.new("RGBA", ori.size, (255, 255, 255))
            new.paste(ori, (0, 0), ori)
            new.save(fout, quality=PIC_QUALITY)
        else:
    ...
    ret = exec_wrap(s3.delete_object)(message, bucket, key, None, None)
    if ret["error"] != None:
        return ret
...
def get_info_from(event):
    ...
    fout = "{}/{}".format(PICTURE_DIR, pic_name.split(".")[0]+".jpg")
